...
# Default demo center coordinates so the map view has something realistic to show.
DEFAULT_CENTER = (12.2958, 76.6394)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        volunteer = User.objects.create_user(
            ...
            lat=DEFAULT_CENTER[0] + 0.01, lng=DEFAULT_CENTER[1] + 0.01,
        )
        ...
        bank_user = User.objects.create_user(
            ...
            lat=DEFAULT_CENTER[0] + 0.02, lng=DEFAULT_CENTER[1],
        )
        general = User.objects.create_user(
            "meera", "meera@example.com", "password123",
            role="general", first_name="Meera", is_verified=True,
            lat=DEFAULT_CENTER[0], lng=DEFAULT_CENTER[1] + 0.02,
        )

        BloodBank.objects.create(
            user=bank_user, name="CityCare Blood Bank",
            address_text="Central District", lat=DEFAULT_CENTER[0] + 0.02, lng=DEFAULT_CENTER[1],
            contact_phone="+91 90000 00000",
        )

        Resource.objects.create(
            owner=donor, title="Study table + chair", category="furniture", condition="good",
            description="Barely used, downsizing household.", quantity=1,
            address_text="West District", lat=DEFAULT_CENTER[0], lng=DEFAULT_CENTER[1],
        )
        Resource.objects.create(
            owner=donor, title="Winter jackets (set of 4)", category="clothes", condition="good",
            description="Kids' sizes 8-12 years.", quantity=4,
            address_text="West District", lat=DEFAULT_CENTER[0] + 0.005, lng=DEFAULT_CENTER[1] - 0.005,
        )
        ...
        FoodListing.objects.create(
            ...
            address_text="North District", lat=DEFAULT_CENTER[0] - 0.01, lng=DEFAULT_CENTER[1] - 0.01,
        )

        BloodRequest.objects.create(
            requester=general, blood_group="O+", units_needed=2, urgency="high",
            hospital_name="Central General Hospital", address_text="Central District",
            lat=DEFAULT_CENTER[0] + 0.015, lng=DEFAULT_CENTER[1] + 0.003,
        ...
        EmergencyRequest.objects.create(
            ...
            address_text="East District", lat=DEFAULT_CENTER[0] - 0.02, lng=DEFAULT_CENTER[1] + 0.015,
        )
