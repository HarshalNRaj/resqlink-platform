    ...
    <div className="min-h-screen bg-paper">
      <header className="mx-auto flex max-w-6xl items-center justify-between px-6 py-6">
        <div className="flex items-center gap-2">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary-500 font-display text-base font-bold text-white">
            R
          </div>
          <span className="font-display text-xl font-bold text-ink">ResQLink</span>
        </div>
        <div className="flex items-center gap-3">
          <Link to="/login" className="text-sm font-semibold text-ink-soft hover:text-ink">Sign in</Link>
          <Link to="/register" className="rounded-lg bg-primary-500 px-4 py-2 text-sm font-semibold text-white hover:bg-primary-600">
            Get started
          </Link>
        </div>
      </header>
      ...
      <section className="mx-auto max-w-6xl px-6 pb-16 pt-10 md:pt-16">
        <div className="grid gap-12 md:grid-cols-2 md:items-center">
          <div>
            <span className="inline-block rounded-full bg-primary-50 px-3 py-1 text-xs font-bold uppercase tracking-wide text-primary-700">
              Community infrastructure · Mysuru
            </span>
            <h1 className="mt-5 font-display text-4xl font-bold leading-tight text-ink md:text-5xl">
              What's unused, wasted.
              <br />
              What's needed, disconnected.
              <br />
              <span className="text-primary-500">ResQLink closes the loop.</span>
            </h1>
            <p className="mt-5 max-w-lg text-ink-soft">
              One platform for donation, food rescue, blood coordination, and emergency
              support — instead of four fragmented apps, phone trees, and social media
              appeals nobody can verify.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/register" className="inline-flex items-center gap-2 rounded-lg bg-primary-500 px-5 py-3 text-sm font-semibold text-white hover:bg-primary-600">
                Create your account <ArrowRight size={16} />
              </Link>
              <Link to="/login" className="inline-flex items-center gap-2 rounded-lg border border-line bg-surface px-5 py-3 text-sm font-semibold text-ink hover:border-primary-500">
                I already have one
              </Link>
            </div>
          </div>

          {/* Signature element: the real 4-stage lifecycle every listing goes through */}
          <div className="rounded-2xl border border-line bg-surface p-6 shadow-sm">
            <p className="mb-5 font-display text-sm font-semibold uppercase tracking-wide text-ink-soft">
