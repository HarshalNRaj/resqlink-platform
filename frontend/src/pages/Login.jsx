  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-paper px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="mt-4 rounded-xl border border-line bg-surface p-3 text-center">
          <p className="text-xs font-medium text-ink-soft mb-2">Quick sign in with demo accounts:</p>
          <div className="flex flex-wrap justify-center gap-1.5">
            {[
              ...
              { label: "User (Meera)", user: "meera" },
            ].map(({ label, user }) => (
              <button
                key={user}
                type="button"
                onClick={() => setForm({ username: user, password: "password123" })}
                className="rounded-md border border-primary-100 bg-primary-50 px-2 py-1 text-xs font-semibold text-primary-700 hover:bg-primary-100 transition-colors"
              >
                {label}
              </button>
            ))}
          </div>
          <p className="mt-2 text-[11px] text-ink-soft/70">Password: password123</p>
        </div>
      </div>
    </div>
  );
