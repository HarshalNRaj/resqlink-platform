...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetches = [
      resources.list({}).catch(() => ({ data: [] })),
      food.list({}).catch(() => ({ data: [] })),
      blood.list({}).catch(() => ({ data: [] })),
      emergency.list({}).catch(() => ({ data: [] })),
    ];

    if (isBB) {
      fetches.push(blood.banks().catch(() => ({ data: [] })));
    } else if (isAdm) {
      fetches.push(authApi.pendingVerifications().catch(() => ({ data: [] })));
    }
